...
def decrypt(encrypted, key):
    ...
    for chunk in chunks:
        ...
        decrypted = decrypted + state_to_hex(state_matrix)

    #remove padding
    decrypted = bytes.fromhex(decrypted)[:-bytes.fromhex(decrypted)[-1]].decode('utf-8')
    
    return decrypted
    
def encrypt(decrypted, key = False):
    provided = key
    if not key:
    ...
    if not provided:
        return key.hex(), base64.b64encode(encrypted.encode('utf-8'))
    else:
