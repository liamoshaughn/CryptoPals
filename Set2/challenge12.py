...
Key = bytes.fromhex(secrets.token_hex(16))
...
def encryption_oracle(user_input):   
    ...
    user_input += base64.b64decode("Um9sbGluJyBpbiBteSA1LjAKV2l0aCBteSByYWctdG9wIGRvd24gc28gbXkgaGFpciBjYW4gYmxvdwpUaGUgZ2lybGllcyBvbiBzdGFuZGJ5IHdhdmluZyBqdXN0IHRvIHNheSBoaQpEaWQgeW91IHN0b3A/IE5vLCBJIGp1c3QgZHJvdmUgYnkK").decode('utf-8')
...
def find_block_size():
    base_len = len(encryption_oracle(""))
    ...
    for i in range(1, 64):
        ...
        if new_len > base_len:
            ...
            #output is a hex string so every two characters is one byte
            return int(block_size / 2), int(base_len / 2)
        

def decryption_oracle(block_size, base_len):
    
    decoded = ""
    
    base_len = int(base_len/16)

    for chunk in range(base_len):
        for j in range(block_size):
            
            plaintext = "A" * (block_size - 1 - j)
            
            cipher_to_search =  encryption_oracle(plaintext)
            cipher_chunks = [cipher_to_search[i:i + block_size * 2] for i in range(0, len(cipher_to_search), block_size * 2)]
            to_search = cipher_chunks[chunk]
            for i in range(255):
                cipher = encryption_oracle(plaintext + decoded + chr(i))
                #block_size*2 here as our cipher is again returned as a hex string so we have our block_size * 2 so that we take in every second character as 1 byte
                chunks = [cipher[i:i + block_size * 2] for i in range(0, len(cipher), block_size * 2)]
                
                if to_search == chunks[chunk]:
                    decoded += chr(i)
                    break
...
def main():
    ...
    repeated_text = ""
    ...
    block_size, base_len = find_block_size()
    ...
    if number_of_duplicates > 1:
        print(decryption_oracle(block_size, base_len))
